"""
Continual learning BUT task switch occurs once
Train on task A until set N value is reached, then
switch to task B and begin training at the N reached on prev. task
i.e. Train DMS until N=10, then switch to Oddball and train starting at N=10, etc.
"""
...
def _build_persistent_optimizer(optimizer_ctor, model: nn.Module):
    """Build one optimizer over all parameters so momentum/state persists across switches."""
...
def _save_advance_checkpoint(model: nn.Module, subdir: str, task_name: str, epoch_idx: int, solved_n: int):
    ...
    ckpt_path = os.path.join(ckpt_dir, f"{task_name}_ctswitch_ep{epoch_idx}_N{solved_n}.pt")
...
def switch_train(
    ...
    """
        Continual learning / task switching training loop.

        Switching is event-driven (not epoch-budget driven):
            - Train the active task until its current N reaches next switch target.
                        - Then switch to the next task in plan order, cycling back to the
                            beginning when the end of the plan is reached.
            - The next switch target is incremented by switch_n each time.
            - The newly active task is initialized to that switch target.
            - Continue until max_global_epochs, target_end_n completion, or plan exhaustion.
    ...
    if switch_n <= 0:
        raise ValueError("switch_n must be > 0.")
    ...
    print(f"[continual] epochs per phase: {epochs_per_phase} (unused for switching)", flush=True)
    ...
    print(f"[continual] switch_n: {switch_n}", flush=True)
    ...
    while global_epoch < max_global_epochs:
        ...
        while global_epoch < max_global_epochs:
            ...
            print(
                ...
                f"phase {phase_idx} | {active_task} | phase_epoch {phase_epoch}] "
                f"loss={mean_loss:.3f} | "
                + " | ".join(
            ...
            # Save stats every 20 epochs
            epochs_since_save += 1
            if epochs_since_save >= save_interval:
            ...
            # Switch task when active task reaches the moving switch target.
            # Next task starts from that target (bounded by maximum_N).
            if len(plan) > 1 and (Ns[active_task][-1] >= next_switch_target):
                next_task_pos = (plan_pos + 1) % len(plan)
                next_task = plan[next_task_pos]
                next_start_n = min(maximum_N, max(task_specs[next_task].get("start_n", 2), next_switch_target))
                solved_N[next_task] = next_start_n
                Ns[next_task] = [next_start_n]

                print(
                    f"[switch] {active_task} reached N={Ns[active_task][-1]} (target={next_switch_target}). "
                    f"Switching to '{next_task}' at N={next_start_n}.",
                    flush=True,
                )

                next_switch_target += switch_n
                plan_pos = next_task_pos
                switched_this_phase = True
                break

        np.save(os.path.join(subdir, "stats_continual.npy"), stats)

        # Stop once all unique tasks reach target in target-capped runs.
        if target_end_n is not None and all(done.values()):
            break

        # Advance phase counter only when moving to next planned task.
        if switched_this_phase:
            phase_idx += 1
            continue

        # Last task continues in same phase index by design.
        phase_idx += 1
