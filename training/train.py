...
def train(
    ...
    if getattr(args, 'resume_ckpt', None) and subdir_override is not None:
        ...
        if os.path.exists(stats_path):
            try:
                ...
                print(f"[resume] Warning: failed to load existing stats ({e}); starting fresh.", flush=True)
    ...
    try:
        ...
        for epoch in tqdm(range(num_epochs)):
            ...
            for j in range(test_steps):
                with torch.no_grad():
                    ...
                    labels    = [l.to(device) for l in labels]
                    _, out_heads = model(sequences, return_timewise=spec["timewise_output"])
                    metric_accumulator.append(spec["metric_fn"](out_heads, labels))
            ...
            metric_name  = metric_accumulator[0].get("name", "score") if metric_accumulator else "score"
            ...
            if metric_accumulator and "endpoint_error" in metric_accumulator[0]:
                mean_ep = float(np.mean([m["endpoint_error"] for m in metric_accumulator]))
                per_ep  = np.mean(np.array([m["per_head_endpoint_error"] for m in metric_accumulator]), axis=0)
                print(f"Aux metric (endpoint_error): {mean_ep:.4f}", flush=True)
                print(
                    "Per-N Aux metric (endpoint_error):\n" +
                    "".join(f"  N={Ns[idx]}: {per_ep[idx]:.4f}\n" for idx in range(len(Ns))),
                    flush=True,
                )
            ...
            metric_for_curriculum = {"score": float(np.mean([m["score"] for m in metric_accumulator]))}
            excluded_keys = {"score", "per_head", "name", "per_head_endpoint_error", "per_head_mse"}
            for k in metric_accumulator[0]:
            ...
            if spec["advance_fn"](metric_for_curriculum):
                if target_end_n is not None and Ns[-1] >= target_end_n:
                    ...
                    print(f"[ADVANCE TRIGGERED] curriculum_type={repr(curriculum_type)} | old_Ns={old_Ns}", flush=True)
                    ...
                    if ct == 'cumulative':
                        Ns = list(Ns) + [int(Ns[-1]) + 1 + i for i in range(args.add_heads)]
                    elif ct == 'sliding':
                        Ns = list(Ns[n_forget:]) + [int(Ns[-1]) + 1 + i for i in range(n_forget)]
                    elif ct == 'single':
                        Ns = [int(Ns[0]) + 1]
        ...
        print(f"\nTraining interrupted. Saving stats to {subdir}/stats.npy...", flush=True)
        np.save(f'{subdir}/stats.npy', stats)
        return stats
...
if __name__ == '__main__':
    ...
    parser.add_argument('--alt_variant', type=str, default='cb_then_rnn',
                        choices=['cb_then_rnn', 'rnn_then_cb_finetune', 'interleaved_finetune',
                                 'interleaved_curr', 'cb_only', 'train_simultaneous', 'cb_only_reservoir'])
    parser.add_argument('--cb_reservoir_n', type=int, default=8)
    parser.add_argument('--th_rnn_lead',    type=float, default=85.0)
    parser.add_argument('--th_cb_lead',     type=float, default=60.0)
    ...
    parser.add_argument('--learning_alg',   type=str, default='bptt')
    ...
    parser.add_argument('--mt_reservoir_mode',    type=str,   default='off',
    ...
    parser.add_argument('--continual_reservoir_mode',   type=str,   default='off',
    ...
    parser.add_argument('--model_type', type=str, default='elman', choices=['elman', 'gru'],
                        help="Type of RNN model to use. 'elman' is a standard Elman RNN, while 'gru' uses GRU cells.")
    ...
    if CURRICULUM == 'cumulative':
        ...
        Ns_init = list(np.arange(START_N, START_N + INIT_HEADS))
    elif CURRICULUM in ('single', 'single_nocurr', 'alternating'):
        Ns_init   = [START_N]
        INIT_HEADS = 1
        NUM_FORGET = 1
    else:
    ...
    AFUNC_MAP = {'leakyrelu': nn.LeakyReLU, 'relu': nn.ReLU, 'sigmoid': nn.Sigmoid, 'tanh': nn.Tanh}
    AFUNC = AFUNC_MAP.get(args.afunc, nn.LeakyReLU)
    if args.afunc and args.afunc not in AFUNC_MAP:
        print(f'Unrecognized activation function: {args.afunc}, defaulting to LeakyReLU')
    ...
    if args.task_switch:
        ...
        for t in plan:
            if t not in TASK_SPECS:
                raise ValueError(f"Unrecognized task in task_switch_plan: {t}")
        ...
        run_number = find_next_free_network_number(
            base_path=BASE_PATH, curriculum_type='task_switch', task='switch',
            affixes=switch_affixes, n_heads=1, n_forget=NUM_FORGET,
        )
        base_subdir = save_model(
            rnn, curriculum_type='task_switch', n_heads=1, n_forget=NUM_FORGET,
            task='switch', network_number=run_number, N_max=MAX_BLOCK_END, N_min=START_N,
            init=True, args=args, base_path=BASE_PATH, affixes=switch_affixes,
        )
        ...
        while True:
            if BLOCK_END >= MAX_BLOCK_END and done_this_block == set(BLOCK_TASKS) and stage_i > 0:
                break
            ...
            startN    = max(int(last_N_by_task.get(stage_task, spec_stage.get("start_n", 2))), MIN_START_N)
            stage_endN = int(min(BLOCK_END, MAX_BLOCK_END))

            if startN >= stage_endN:
                done_this_block.add(stage_task)
                print(f"[STAGE {stage_i}] task={stage_task} already at startN={startN} >= block_target={stage_endN}", flush=True)
            else:
                stage_dir = make_unique_dir(os.path.join(base_subdir, f"switch{stage_i}_{stage_task}"))
                print(
                    f"[STAGE {stage_i}] task={stage_task} startN={startN} -> target_endN={stage_endN} dir={stage_dir}",
                    flush=True,
                )
    ...
    if args.multitask:
        ...
        run_number = find_next_free_network_number(
            base_path=BASE_PATH, curriculum_type='multitask', task='mt',
            affixes=mt_affixes, n_heads=len(MULTITASK_TASKS), n_forget=1,
        )
        mt_subdir = save_model(
            rnn, curriculum_type='multitask', n_heads=len(MULTITASK_TASKS), n_forget=1,
            task='mt', network_number=run_number, N_max=args.mt_target_n, N_min=2,
            init=True, args=args, base_path=BASE_PATH, affixes=mt_affixes,
        )

        cb_param_ids = {id(p) for p in rnn.cb.parameters()} if getattr(rnn, 'cb', None) else set()
    ...
    # ---------------------------------------------------------------
        ...
        run_number   = find_next_free_network_number(
            base_path=BASE_PATH, curriculum_type='continual', task='cont',
            affixes=cont_affixes, n_heads=len(set(plan)), n_forget=1,
        )
        cont_subdir = save_model(
            rnn, curriculum_type='continual', n_heads=len(set(plan)), n_forget=1,
            task='cont', network_number=run_number, N_max=args.continual_target_n, N_min=2,
            init=True, args=args, base_path=BASE_PATH, affixes=cont_affixes,
        )
    ...
    # ---------------------------------------------------------------
        ...
        run_number     = find_next_free_network_number(
            base_path=BASE_PATH, curriculum_type='ct_switch', task='ctswitch',
            affixes=switch_affixes, n_heads=len(set(plan)), n_forget=1,
        )
        switch_subdir = save_model(
            rnn, curriculum_type='ct_switch', n_heads=len(set(plan)), n_forget=1,
            task='ctswitch', network_number=run_number, N_max=args.continual_target_n, N_min=2,
            init=True, args=args, base_path=BASE_PATH, affixes=switch_affixes,
        )
    ...
    if CURRICULUM == 'alternating':
        ...
        if args.resume_subdir is None:
            ...
            run_number = NETWORK_NUMBER
            print(f"=== Resuming Standard Training in: {args.resume_subdir} ===", flush=True)
